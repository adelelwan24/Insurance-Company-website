...
def thanks():
    ...
    if not beneficiary :
        plans = db("select * from plan_type;")
        return render_template('purchaseplanform.html',plans=plans,  ben_name = ben_name ,ben_ssn = ben_ssn ,customer_ssn=customer_ssn, wrong='wrong' , data = 'you must choose a beneficiary')
    ...
    if customer_in == 1 :
        # default is for the id attribute which is int auto increment so it adds after the greatest number in the database
        # db(f"INSERT INTO insurance_plan values(default,CURDATE(), '{type_ben}', '{number_ben}', '{customer_ssn}','{customer_ssn}', '{type}');")
